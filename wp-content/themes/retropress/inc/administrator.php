		...
		<script type="text/javascript">
			jQuery(function() {
				jQuery('ul.tabs-menu').delegate('li:not(.active)', 'click', function() {
					...
					jQuery('#adminmenu .wp-has-current-submenu ul li').removeClass('current').eq(jQuery(this).index()).addClass('current');
...
<?php
	...
	{
		?>
		<script type="text/javascript" src="<?php echo get_template_directory_uri()?>/js/admin.js"></script>
		...
		switch ($param['type']) {
						...
						case 'activator':
							...
							<div style='overflow:hidden'>
								<div class='item' style='width:40%;float:left;padding:0 5%;border-right:1px solid #e3e3e3;padding-bottom:50px;'>
										<?php
											...
											$data='?'.implode('&', $data);
										?>
									<iframe src='http://smthemes.com/purchase-from-users-dashboard/<?php echo $data; ?>' width='100%' height='500px' scrolling='no'>
									</iframe>
									
								</div>
								<div class='item' style='width:40%;float:left;padding:0 5%;margin-left:-1px;'>
									<p class='p_ttl'>
...
<?php
						...
						case 'socials':
							...
							<div class='socialbox' alt='<?php echo $param['name']?>'>
								<table>
									<tr class='th'>
										<th style='width:24px'></th>
	...
	function category() {
		...
		?>
			<div style='font-style:italic;'>Slide thumbnail can be added using the "Set Featured Image" link, located under categories list at post write/edit page.</div>
			<div class='item'>
			...
			<?php
				if (count($slides)&&is_array($slides)) {
					foreach($slides as $num=>$slide) {
						?>
						<dt class='slide_ttl' id="slide_dt_<?php echo $num?>"><img src="<?php echo preg_replace('/(.*)\.(.*)/', '$1-prev.$2', $slide['img'])?>" width="56" height="56" alt="<?php echo $slide['ttl']?>" title="<?php echo $slide['ttl']?>" /></dt>
						<dd class="slide_item" id="slide_dd_<?php echo $num?>">
